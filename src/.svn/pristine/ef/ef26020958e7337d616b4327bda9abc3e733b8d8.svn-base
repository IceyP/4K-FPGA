...
begin
    ...
    ST_IDLE:
    begin
        if((info_fifo_empty==1'b0) && (sl_bp==1'b1))
        begin
...
begin
    ...
    begin
        ...
        8'h00:  sl_data <=  {pkt_type[1:0],tuner_channel[1:0],chacha_encrypt_period[3:0]};
        8'h01:  sl_data <=  chacha_counter[23:16];
        8'h02:  sl_data <=  chacha_counter[15:8];
        8'h03:  sl_data <=  chacha_counter[7:0];
        default:sl_data <=  {8{1'b0}};
        endcase
    end
    else if(st_curr==ST_TS_ENCRYPT)
    begin
        ...
        begin
            if((sc_byte_cnt>9) && (sc_byte_cnt<194))
            begin
                sl_data <=  datram_rdata ^ keyram_rdata;
            end
            else
            begin
                sl_data <=  datram_rdata;
            end
        end
        else
        begin
...
begin
    ...
    begin
        chacha_bypass_counter   <=  tdprama_doutb[3:0];
    end
end
...
cc_encryption u0_cc_encryption(
    ...
    .chacha_ts_ack                      ( chaha_ts_ack              ),  //not used
